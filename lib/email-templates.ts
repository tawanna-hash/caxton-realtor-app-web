...
export function renewalEmail(params: RenewalEmailParams): string {
  ...
  const urgencyColor = daysRemaining <= 7 ? '#dc2626' : daysRemaining <= 14 ? '#f97316' : '#6b7280';
  ...
  const attachBlock = params.agreementDataUri
    ? `<tr><td style="padding:0 40px 24px 40px"><table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#eff6ff;border:2px solid #3b82f6;border-radius:6px"><tr><td style="padding:16px 20px"><p style="margin:0 0 8px 0;font-family:Arial,sans-serif;font-size:12px;font-weight:800;color:#1e40af;text-transform:uppercase;letter-spacing:.8px">&#1d4ed8; Renewal Agreement Attached</p><p style="margin:0 0 12px 0;font-family:Arial,sans-serif;font-size:13px;color:#1d4ed8;line-height:1.6">Your pre-filled renewal agreement is ready to sign. Your current rate of <strong>$${rateStr}/issue</strong> is <strong>locked</strong> — same ad size, frequency, and terms as your original agreement.</p><table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#fff;border:1px solid #93c5fd;border-radius:4px"><tr><td style="padding:12px 16px"><span style="font-size:20px;vertical-align:middle;margin-right:10px">&#1d4ed8;</span><span style="display:inline-block;vertical-align:middle"><strong style="font-family:Arial,sans-serif;font-size:13px;color:#1e40af">Renewal Agreement — ${companyName}</strong><br><span style="font-family:Arial,sans-serif;font-size:11px;color:#6b7280">Rate locked &middot; Same terms &middot; Signature fields ready</span></span></td></tr></table><p style="margin:10px 0 0 0;font-family:Arial,sans-serif;font-size:12px;color:#1d4ed8">&#1d4ed8; <strong>Rate Lock:</strong> Sign before <strong>${params.expirationDate}</strong> to lock in your rate of <strong>$${rateStr}/issue</strong>.</p></td></tr></table></td></tr>`
    : '';
...
<tr><td style="background:#fff7ed;border-bottom:1px solid #fed7aa;padding:12px 40px;text-align:center"><p style="margin:0;font-family:Arial,sans-serif;font-size:14px;color:#555">${urgencyText}</p></td></tr>
...
export function amendedAgreementEmail(params: AmendedAgreementEmailParams): string {
  ...
  const changeBlock = params.changeSummary && params.changeSummary.trim()
    ? `<tr><td style="padding:0 40px 24px 40px"><table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#fff7ed;border:1px solid #fed7aa;border-radius:4px"><tr><td style="padding:16px 20px"><p style="margin:0 0 6px 0;font-family:Arial,sans-serif;font-size:12px;font-weight:800;color:#9a3412;text-transform:uppercase;letter-spacing:.8px">What changed</p><p style="margin:0;font-family:Arial,sans-serif;font-size:14px;color:#9a3412;line-height:1.6;white-space:pre-wrap">${escapeHtml(params.changeSummary.trim())}</p></td></tr></table></td></tr>`
    : '';
...
<tr><td style="background:#f3f4f6;border-top:1px solid #e5e7eb;padding:20px 40px;text-align:center"><p style="margin:0;font-family:Arial,sans-serif;font-size:12px;color:#888">${brand.brandName} | <a href="${websiteUrl}" style="color:#888;text-decoration:none">${websiteUrl}</a></p></td></tr>
</table></td></tr></table></body></html>`;
}
