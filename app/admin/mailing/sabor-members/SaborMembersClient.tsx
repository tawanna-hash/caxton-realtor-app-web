...
//
// Client UI for SABOR Members (holding-stage mailing rows from ramco.sabor.com).
// Mirrors the ABOR Members layout exactly but is a fully standalone page:
//   - hardcoded source=ramco-sabor (no source filter dropdown)
//   - Sync button triggers the SABOR GH Actions dispatch
//   - Sync metadata banner (last run, totals, cookie freshness)
...
//   - "Sync SABOR Now" button (dispatches GitHub Actions workflow)
...
export default function SaborMembersClient() {
  ...
  // Sync metadata banner state.
  const [syncStatus, setSyncStatus] = useState<SaborSyncStatus | null>(null);

  const [search, setSearch] = useState<string>('');
  const [filter, setFilter] = useState<FilterKey>('all');
  // Source is hardcoded to 'ramco-sabor' — this page is SABOR-only.
  const source = 'ramco-sabor';
  ...
  const syncSaborNow = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(j?.detail || j?.error || `HTTP ${res.status}`);
      showToast(
        'Sync dispatched. GitHub Actions run started — expect new members in ~90 minutes. Refresh to see progress.',
      );
    } catch (err) {
  ...
  // (mirrors the segment-page toolbar but scoped to SABOR holding rows).
  // ------------------------------------------------------------------
  const HOLDING_SOURCE = 'ramco-sabor';
  const AUDIENCE_LABEL = 'SABOR Members';
    ...
    <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="rounded-md border border-gray-200 bg-white p-4 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {syncStatus.meta.last_status ?? 'no runs yet'}
            </p>
          </div>
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {syncStatus.meta.last_total !== null
                ? `${syncStatus.meta.last_total.toLocaleString()} processed`
                : 'no runs yet'}
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">GitHub Actions</p>
          </div>
        </div>
      )}
        ...
        <KpiCard
          ...
          action={(counts?.near ?? 0) > 0 ? {
            ...
            onClick: () => {
              ...
              window.location.href = '/api/admin/mailing/holding/export-near?source=ramco-sabor';
            },
          } : undefined}
        />
      ...
      <div className="sm:hidden rounded-md border border-gray-200 bg-white divide-y divide-gray-100">
        ...
        {!loading && rows.length === 0 && (
          <div className="px-3 py-8 text-center text-sm text-gray-500">No members. Try “Sync SABOR Now” above. The first run takes ~90 minutes.</div>
        )}
        {!loading && rows.map((r) => {
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {!loading && rows.length === 0 && (
              <tr><td colSpan={9} className="px-3 py-8 text-center text-gray-500">
                No members. Try “Sync SABOR Now” above. The first run takes ~90 minutes.
              </td></tr>
            )}
            {!loading && rows.map((r) => {
              const hasAddr = !!(r.address || r.city || r.state || r.zip);
