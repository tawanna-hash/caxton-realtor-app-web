...
type Item = {
  ...
  website: string | null;
  phone: string | null;
  status: 'pending' | 'realtyline' | 'san_antonio' | 'rejected';
};
...
export default function FastEmailRealtorsClient() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`/api/admin/content/fastemail-realtors?status=${filter}`, {
        credentials: 'include',
      });
      ...
      setNotice(e instanceof Error ? e.message : 'Unable to load review queue.');
    }
  }, [filter]);
  useEffect(() => {
  ...
  const rows = useMemo(() => {
    ...
    const value = (row: Item, key: SortKey) =>
      key === 'received_at' ? row.received_at : row[key] || '';
    return data.rows
      .filter(
        (row) =>
          !needle ||
          [row.agent_name, row.company, row.email, row.website, row.phone, row.business_address].some(
            (v) => v?.toLowerCase().includes(needle),
          ),
      )
      .sort(
        (a, b) =>
          String(value(a, sort.key)).localeCompare(String(value(b, sort.key)), undefined, {
            numeric: true,
            sensitivity: 'base',
  ...
  const header = (key: SortKey, label: string) => (
    <button className="font-medium hover:text-gray-900" onClick={() => toggleSort(key)}>
      {label}
  ...
  async function scan() {
    ...
    setNotice(null);
    try {
      const res = await fetch('/api/admin/content/fastemail-realtors', {
        method: 'POST',
        credentials: 'include',
      });
      const json = await res.json();
      ...
      setNotice(
        `Scan complete: ${json.queued} queued; ${json.repaired || 0} repaired; ${json.skipped} skipped.`,
      );
      await load();
    } catch (e) {
  ...
  async function act(id: string, action: 'realtyline' | 'san_antonio' | 'reject') {
    ...
    try {
      ...
      setNotice(
        json.duplicate
          ? 'The email already exists in that mailing list; review record was updated.'
          : action === 'reject'
            ? 'Record rejected.'
            : 'Contact added to the selected mailing list.',
      );
      await load();
    } catch (e) {
  ...
  async function deleteSelected() {
    ...
    if (
      !ids.length ||
      !window.confirm(
        `Delete ${ids.length} selected review record${ids.length === 1 ? '' : 's'}? This cannot be undone.`,
      )
    )
      return;
    setBusy('delete');
    setNotice(null);
    try {
      const res = await fetch('/api/admin/content/fastemail-realtors/delete', {
        method: 'DELETE',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
      ...
      setNotice(`${json.deleted} review record${json.deleted === 1 ? '' : 's'} deleted.`);
      await load();
    } catch (e) {
  ...
  const renderActions = (row: Item) =>
    row.status === 'pending' ? (
      ...
      <span className="text-xs uppercase tracking-wide text-gray-500">{labels[row.status]}</span>
    );
    ...
    <main className="max-w-6xl mx-auto px-6 py-8 space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <PageTitle size="md">FastEmail Realtor Review</PageTitle>
          <p className="mt-2 text-sm text-gray-600 max-w-2xl">
            Review realtor contact signatures from Fast Email Flyers. Property addresses are
            excluded. Contacts are only added after you choose a mailing list.
          </p>
        </div>
        <button
          onClick={scan}
          disabled={busy !== null}
          className="px-4 py-2 rounded-md bg-brand-700 text-white text-sm font-medium disabled:opacity-50"
        >
      ...
      <div className="flex gap-2 flex-wrap">
        {['pending', 'realtyline', 'san_antonio', 'rejected', 'all'].map((key) => (
          <button
            key={key}
      ...
      <ul className="sm:hidden divide-y divide-gray-100 rounded-md border border-gray-200 bg-white overflow-hidden">
        <li className="flex items-center gap-2 px-3 py-2 bg-gray-50">
          <input
            id="fastemail-select-all-mobile"
            type="checkbox"
          ...
          />
          <label
            htmlFor="fastemail-select-all-mobile"
          ...
          rows.map((row) => (
            <li key={`m-${row.id}`} className="p-3">
              <div className="flex items-start gap-3">
                <input
                  type="checkbox"
                  aria-label={`Select ${row.agent_name || row.email || 'contact'}`}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <dl className="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
                    ...
                    <dd className="text-gray-700 truncate">{row.company || '—'}</dd>
                    ...
                    {row.phone && (
                      <>
                        ...
                        <dd className="text-gray-700">{row.phone}</dd>
                      </>
                    )}
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50 text-left text-xs uppercase text-gray-600">
            <tr>
              ...
              <th className="px-3 py-3">{header('agent_name', 'Realtor')}</th>
              <th className="px-3 py-3">{header('company', 'Company')}</th>
              <th className="px-3 py-3">Contact</th>
              <th className="px-3 py-3">{header('website', 'Website')}</th>
              <th className="px-3 py-3">Business address</th>
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="px-3 py-3">{renderActions(row)}</td>
                </tr>
              ))
            )}
          </tbody>
