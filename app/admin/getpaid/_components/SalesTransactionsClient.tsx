        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {rows.map((invoice) => {
              ...
              return <tr key={invoice.id} className="hover:bg-gray-50"><td className="whitespace-nowrap px-4 py-3">{formatTransactionDate(invoice.issued_at ?? invoice.created_at)}</td><td className="px-4 py-3">{invoice.number?.startsWith('SR-') ? 'Sales receipt' : 'Invoice'}</td><td className="px-4 py-3">{invoice.number ?? 'Draft'}</td><td className="px-4 py-3">{invoice.advertiser_name ?? invoice.bill_to_name ?? '—'}</td><td className="max-w-xs truncate px-4 py-3 text-gray-500" title={description}>{description}</td><td className="px-4 py-3 text-right">{formatCents(invoice.total_cents)}</td><td className="px-4 py-3 capitalize">{invoice.is_overdue ? 'Overdue' : invoice.status}</td><td className="px-4 py-3 text-right"><Link className="text-orange-700 hover:underline" href={`/admin/getpaid/invoices?edit=${invoice.id}`}>View</Link></td></tr>;
            })}
          </tbody>
        </table>
