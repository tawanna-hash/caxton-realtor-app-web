    ...
    <div className="relative">
      <button type="button" onClick={() => { setOpen(!open); setError(''); }}
        ...
        <span className="flex min-w-0 items-center gap-2">
          ...
          ) : <span aria-hidden="true" className="h-8 w-8 shrink-0 rounded-full bg-gray-100" />}
                ...
                {results.map((author) => (
                  <li key={author.name} role="option" aria-selected={name.toLowerCase() === author.name.toLowerCase()}>
                    <button type="button" onClick={() => { onChange(author); setOpen(false); }}
                      className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm hover:bg-gray-100 focus-visible:bg-gray-100">
                      {author.avatar ? (
                        // eslint-disable-next-line @next/next/no-img-element
