          ...
          <div className="grid items-stretch gap-5 border border-[#D9D0BF] bg-[#FFFDF8] p-5 lg:grid-cols-3 lg:p-6">
              ...
              <div className="mt-5 space-y-2">
                {!radarItems.length ? <div className="border border-dashed border-slate-300 bg-white p-5 text-sm leading-6 text-slate-600">Add a transaction, effective date, and closing date to set this Date Radar window.</div> : radarItems.map((item) => (
                  <button type="button" key={item.id} onClick={() => focusDeal(item.dealId)} className="flex w-full items-center gap-3 border border-slate-200 bg-white p-3 text-left transition hover:border-[#7059A8]">
                    <span className={`h-2.5 w-2.5 shrink-0 rounded-xl ${item.overdue ? 'bg-[#B6402C]' : item.kind === 'deadline' ? 'bg-[#7059A8]' : 'bg-[#C88A14]'}`} />
            ...
            <div className="order-2 h-full border border-slate-200 bg-white p-5 sm:p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button type="button" onClick={exportActiveDealCalendar} disabled={!activeDeal || !calendarEventsForDeal(activeDeal).length} className="inline-flex min-h-[42px] items-center gap-2 rounded-xl bg-[#301D5D] px-4 text-sm font-bold text-white transition hover:bg-[#42277c] disabled:cursor-not-allowed disabled:opacity-45"><Download className="h-4 w-4" aria-hidden="true" />Export this deal</button>
                <button type="button" onClick={exportAllDealsCalendar} disabled={!deals.some((deal) => deal.status !== 'completed' && calendarEventsForDeal(deal).length)} className="inline-flex min-h-[42px] items-center gap-2 rounded-xl border border-[#7059A8] bg-white px-4 text-sm font-bold text-[#301D5D] transition hover:bg-[#F8F5FF] disabled:cursor-not-allowed disabled:opacity-45"><CalendarDays className="h-4 w-4" aria-hidden="true" />Export active deals</button>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 lg:py-10">
          {!activeDeal ? (
            <div className="border border-dashed border-slate-300 bg-white p-6 text-center">
              ...
              <button type="button" onClick={createDeal} className="mt-5 inline-flex min-h-[42px] items-center gap-2 rounded-xl bg-[#301D5D] px-4 text-sm font-bold text-white">
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                <div className="border border-slate-200 bg-white p-5 sm:p-6">
                  ...
                  <div className="mt-5 grid min-w-0 gap-3 sm:grid-cols-2">
                    ...
                    <button type="button" onClick={addTask} className="inline-flex min-h-[44px] items-center justify-center gap-2 rounded-xl bg-[#301D5D] px-4 text-sm font-bold text-white"><Plus className="h-4 w-4" aria-hidden="true" />Add</button>
                  </div>
                  <div className="mt-4 grid min-w-0 gap-2 border-y border-slate-100 py-4 sm:grid-cols-2">
                    ...
                    <button type="button" onClick={addCustomReminder} disabled={!reminderDeadlineId || !reminderDate} className="inline-flex min-h-[42px] items-center justify-center rounded-xl border border-[#7059A8] px-4 text-sm font-bold text-[#301D5D] disabled:opacity-40">Add reminder</button>
                  </div>
                  <div className="mt-5 space-y-2">
      ...
      <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 lg:py-16">
        ...
        <nav aria-label="Deal Desktop pages" className="mt-5 border border-slate-200 bg-white p-4">
          ...
          {workspacePage === 2 ? (
            ...
            >
              <div className="relative sm:w-[228px]">
                <div
                  className={`flex h-[42px] overflow-hidden rounded-xl border text-sm font-bold transition ${
                ...
                {isUploadMenuOpen && extractionState !== 'extracting' && (
                  <div
                    role="menu"
                    className="absolute right-0 z-20 mt-2 w-[280px] rounded-xl border border-slate-200 bg-white p-2 text-left shadow-lg"
                  >
                    <label
                      htmlFor="agentContractUploadMenu"
                      role="menuitem"
                      onClick={() => setIsUploadMenuOpen(false)}
                      className="flex cursor-pointer items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold text-slate-800 transition hover:bg-violet-50"
                    >
            ...
            <div className="flex flex-wrap gap-2">
              <Link href="/agents" className="inline-flex min-h-[42px] items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-bold text-slate-700 hover:border-[#301D5D]">
                <ChevronLeft className="h-4 w-4" aria-hidden="true" /> Deal Desktop
              </Link>
              <button type="button" onClick={saveProgress} disabled={!ready || syncState === 'saving'} className="inline-flex min-h-[42px] items-center gap-2 rounded-xl border border-[#7059A8] bg-white px-4 text-sm font-bold text-[#301D5D] disabled:opacity-50">
                <Save className="h-4 w-4" aria-hidden="true" /> {syncState === 'saving' ? 'Saving…' : 'Save for later'}
              </button>
              <button type="button" onClick={() => setWorkspacePage(2)} className="inline-flex min-h-[42px] items-center gap-2 rounded-xl bg-[#301D5D] px-4 text-sm font-bold text-white hover:bg-[#42277c]">
          ...
          <div
            className="mt-4 h-2 overflow-hidden rounded-xl bg-slate-100"
          ...
          >
            <div
              className="h-full rounded-xl bg-[#7059A8] transition-[width]"
          ...
          {workspacePage === 2 && (
            <div className="mt-3 grid gap-2 sm:ml-auto sm:max-w-[504px] sm:grid-cols-3" aria-label="Worksheet navigation">
              {activeDeal && activeDeal.worksheetStep > 0 ? (
                <button type="button" onClick={() => setWorksheetStep(activeDeal.worksheetStep - 1)} className="inline-flex min-h-[42px] w-full items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-bold text-slate-700 transition hover:border-[#301D5D] hover:bg-[#F8F5FF]">
                  <ChevronLeft className="h-4 w-4" aria-hidden="true" /> Back
                </button>
              ) : (
                <button type="button" onClick={() => setWorkspacePage(1)} className="inline-flex min-h-[42px] w-full items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 text-sm font-bold text-slate-700 transition hover:border-[#301D5D] hover:bg-[#F8F5FF]">
                  <ChevronLeft className="h-4 w-4" aria-hidden="true" /> Back
                </button>
              )}
              <button type="button" onClick={saveProgress} disabled={!ready || syncState === 'saving'} className="inline-flex min-h-[42px] w-full items-center justify-center gap-2 rounded-xl border border-[#7059A8] bg-white px-4 text-sm font-bold text-[#301D5D] transition hover:bg-[#F8F5FF] disabled:opacity-50">
                <Save className="h-4 w-4" aria-hidden="true" /> {syncState === 'saving' ? 'Saving…' : 'Save for later'}
              </button>
              {activeDeal && activeDeal.worksheetStep < WORKSHEET_STEPS.length - 1 ? (
                <button type="button" onClick={() => setWorksheetStep(activeDeal.worksheetStep + 1)} className="inline-flex min-h-[42px] w-full items-center justify-center gap-2 rounded-xl bg-[#301D5D] px-4 text-sm font-bold text-white transition hover:bg-[#42277c]">
                  Next step <ChevronRight className="h-4 w-4" aria-hidden="true" />
                </button>
              ) : (
                <Link href="/agents" className="inline-flex min-h-[42px] w-full items-center justify-center gap-2 rounded-xl bg-[#301D5D] px-4 text-sm font-bold text-white transition hover:bg-[#42277c]">
          ...
          {[
            ...
            return (
              <div key={label as string} className="border border-slate-200 bg-white p-4">
                <div className={`flex h-9 w-9 items-center justify-center rounded-xl ${tone as string}`}>
          ...
          <div className="order-2 h-full border border-slate-200 bg-white p-5 sm:p-6">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button
                ...
                className="inline-flex min-h-[42px] items-center gap-2 rounded-xl bg-[#301D5D] px-4 text-sm font-bold text-white transition hover:bg-[#42277c] disabled:cursor-not-allowed disabled:opacity-45"
              >
              ...
              <button
                ...
                className="inline-flex min-h-[42px] items-center gap-2 rounded-xl border border-[#7059A8] bg-white px-4 text-sm font-bold text-[#301D5D] transition hover:bg-[#F8F5FF] disabled:cursor-not-allowed disabled:opacity-45"
              >
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <PushOptInButton
                  ...
                  className="inline-flex min-h-[36px] items-center rounded-xl border border-[#7059A8] bg-white px-3 text-xs font-bold text-[#301D5D] transition hover:bg-[#F8F5FF]"
                />
              </div>
              <div className="flex flex-wrap gap-x-4 gap-y-2 border-t border-slate-100 pt-3">
                {([
          ...
          <div id="current-transaction" className="scroll-mt-24 border border-slate-200 bg-white p-5 sm:p-7">
                  ...
                  {activeDeal && (
                    pendingRemoval === activeDeal.id ? (
                      <button
                        ...
                        className="inline-flex min-h-[42px] items-center gap-2 rounded-xl bg-[#9A3D2B] px-4 text-sm font-bold text-white"
                      >
                        Confirm remove
                      </button>
                    ) : (
                      <button
                        ...
                        className="inline-flex min-h-[42px] items-center gap-2 rounded-xl border border-[#D8A79D] px-4 text-sm font-semibold text-[#9A3D2B] transition hover:bg-[#FFF0EC]"
                      >
            ...
            {!activeDeal ? (
              <div className="mt-7 flex min-h-[260px] flex-col items-center justify-center border border-dashed border-slate-300 bg-[#FCFBF9] px-6 text-center">
                ...
                <button type="button" onClick={createDeal} className="mt-5 inline-flex min-h-[44px] items-center gap-2 rounded-xl bg-[#301D5D] px-4 text-sm font-bold text-white">
                  Create transaction
                    ...
                    <section role="status" className="mt-4 border border-emerald-200 bg-emerald-50 p-4">
                      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                        ...
                        <div className="flex shrink-0 flex-wrap gap-2">
                          <button type="button" onClick={applyExtraction} className="inline-flex min-h-[40px] items-center justify-center rounded-xl bg-emerald-700 px-4 text-sm font-bold text-white hover:bg-emerald-800">Apply to this deal</button>
                          <button type="button" onClick={() => { setExtractionDraft(null); setExtractionState('idle'); }} className="inline-flex min-h-[40px] items-center justify-center rounded-xl border border-emerald-300 bg-white px-4 text-sm font-bold text-emerald-800 hover:bg-emerald-100">Discard</button>
                        </div>
                      </div>
