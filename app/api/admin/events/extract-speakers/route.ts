...
import { extractEventSpeakersPage } from '@/lib/server/gemini-event-flyer-extract';
...
export const POST = withAdminTracking(async function POST(request: NextRequest) {
  ...
  await rateLimit('general', 'event-speaker-extract');
  let form: FormData;
  try {
  ...
  if (!(file instanceof File) || !ALLOWED_MIME.has(file.type)) {
    return NextResponse.json({ error: 'Upload a JPG, PNG, or WebP speaker page.' }, { status: 415 });
  }
  if (file.size === 0 || file.size > MAX_BYTES) {
    return NextResponse.json({ error: 'Speaker page must be 3 MB or smaller.' }, { status: 413 });
  }
  ...
  const result = await extractEventSpeakersPage({ imageBase64: bytes.toString('base64'), mimeType: file.type });
  if (!result.ok) {
    return NextResponse.json(
      { error: result.reason === 'rate-limit' ? 'Speaker reader is busy. Please retry shortly.' : 'Could not read this speaker page.', reason: result.reason },
      { status: result.reason === 'rate-limit' ? 429 : result.reason === 'no-key' ? 503 : 502 },
    );
  }
  return NextResponse.json({ speakers: result.speakers }, { headers: { 'Cache-Control': 'no-store' } });
});
