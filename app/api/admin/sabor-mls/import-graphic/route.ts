/**
 * POST /api/admin/sabor-mls/import-graphic
 *
 * Multipart/form-data:
 *   field `image` — a PNG/JPEG/WEBP raster or a PDF of the monthly SABOR
 *                   Market Stats infographic.
 *
 * Returns:
 *   { ok: true, extracted: ExtractedSaborReport }
 *
 * The client merges `extracted` into the editor's form state; every
 * field on ExtractedSaborReport is optional so unedited state is
 * preserved.
 */
...
// Gemini vision + PDF calls can push past the default 10s.
export const maxDuration = 60;

const MAX_BYTES = 20 * 1024 * 1024; // 20 MB — PDFs can be chunky
...
export const POST = withAdminTracking(async function POST(req: NextRequest) {
  ...
  const result = await extractFromSaborGraphic({ imageBase64, mimeType });
  if (!result.ok) {
    captureServerEvent('sabor_import_failed', admin.email ?? 'server', {
      surface: 'admin_sabor_mls',
    ...
    return NextResponse.json(
      { ok: false, error: `extraction failed: ${result.reason}`, detail: result.detail },
      { status },
    );
  }

  const d = result.data;
  captureServerEvent('sabor_import_succeeded', admin.email ?? 'server', {
    surface: 'admin_sabor_mls',
    mime_type: mimeType,
    byte_size: file.size,
