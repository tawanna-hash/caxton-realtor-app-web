...
function validatePatch(body: unknown): { ok: true; data: Partial<SaborReport> } | { ok: false; error: string } {
  ...
  const patch = Object.fromEntries(Object.entries(data).filter(([key]) => allowed.has(key))) as Partial<SaborReport>;
  return Object.keys(patch).length ? { ok: true, data: patch } : { ok: false, error: 'no fields to update' };
}

async function updatePatch(id: number, data: Partial<SaborReport>) {
  const columns: Record<keyof SaborReport, string> = { month_label: 'month_label', month_label_es: 'month_label_es', released_at: 'released_at', subtitle_en: 'subtitle_en', subtitle_es: 'subtitle_es', headline_value: 'headline_value', headline_delta: 'headline_delta', headline_delta_direction: 'headline_delta_direction', headline_label_en: 'headline_label_en', headline_label_es: 'headline_label_es', indicator_stats: 'indicator_stats', listing_counts: 'listing_counts', price_bands: 'price_bands', page_count: 'page_count', pdf_storage_key: 'pdf_storage_key' };
  const values: unknown[] = [];
  const clauses: string[] = [];
  for (const [key, column] of Object.entries(columns) as [keyof SaborReport, string][]) {
  ...
  values.push(id);
  await getSql().query(
    `UPDATE sabor_mls_reports SET ${clauses.join(', ')}, updated_at = NOW() WHERE id = $${values.length}`,
    values,
  );
}
