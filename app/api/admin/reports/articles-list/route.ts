...
// Helper endpoint for the report builder UI. Returns the list of distinct
// articles that have at least one tracked event in the rolling window, with
// their title and pub. Used to populate the article dropdown so the user
// picks from real data instead of typing IDs.
//
// Resilience: PostHog and WP are queried independently. Either failing must
// not blank out the picker. Always returns 200 with whatever subset of
// articles we could assemble, plus an optional `warning` field.
...
async function fetchPostHogRows(days: number): Promise<{
  ...
  try {
    ...
    console.error('[articles-list] PostHog failed:', err);
    return { byId: new Map(), warning: 'Open-count data unavailable.' };
  }
}
...
async function fetchLiveArticles(): Promise<{ live: LiveRow[]; warning: string | null }> {
  ...
  try {
    ...
    console.error('[articles-list] austin failed:', err);
    warnings.push('Austin article list unavailable.');
  }
  try {
    const sa = await getNewsRaw('san_antonio');
    for (const a of sa) live.push({ id: String(a.id), head: a.head, pub: 'newsline' });
  } catch (err) {
    console.error('[articles-list] sa failed:', err);
    warnings.push('San Antonio article list unavailable.');
  }
...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('[articles-list] auth threw:', authErr);
    }
    if (!isAdmin) {
