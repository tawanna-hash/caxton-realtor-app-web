...
export async function GET(_req: NextRequest, ctx: RouteCtx) {
  ...
  try {
    ...
    `;
    return NextResponse.json({ schedule: rows[0], generated_invoices: invoices });
  } catch (err) {
...
export const PATCH = withAdminTracking(async function PATCH(req: NextRequest, ctx: RouteCtx) {
  ...
  try {
    await ensureSchema();
    const sql = getSql();
    const existing = (await sql`SELECT id FROM recurring_invoice_schedules WHERE id = ${id}`) as unknown as Array<{ id: string }>;
    ...
    for (const field of RECURRING_SCHEDULE_PATCHABLE_FIELDS) {
      ...
      if (field === 'status' && typeof raw === 'string' && !RECURRING_SCHEDULE_STATUS_VALUES.has(raw as never)) continue;
      if (field === 'frequency' && typeof raw === 'string' && !RECURRING_FREQUENCY_VALUES.has(raw as never)) continue;

      switch (field) {
        case 'name':                await sql`UPDATE recurring_invoice_schedules SET name = ${raw as string}                              WHERE id = ${id}`; break;
        case 'status':               await sql`UPDATE recurring_invoice_schedules SET status = ${raw as string}                            WHERE id = ${id}`; break;
        case 'frequency':            await sql`UPDATE recurring_invoice_schedules SET frequency = ${raw as string}                         WHERE id = ${id}`; break;
        case 'interval_count':       await sql`UPDATE recurring_invoice_schedules SET interval_count = ${raw as number}                    WHERE id = ${id}`; break;
        case 'day_of_month':         await sql`UPDATE recurring_invoice_schedules SET day_of_month = ${raw as number | null}                WHERE id = ${id}`; break;
        case 'amount_cents':         await sql`UPDATE recurring_invoice_schedules SET amount_cents = ${raw as number}                       WHERE id = ${id}`; break;
        case 'tax_cents':            await sql`UPDATE recurring_invoice_schedules SET tax_cents = ${raw as number}                          WHERE id = ${id}`; break;
        case 'line_items':           await sql`UPDATE recurring_invoice_schedules SET line_items = ${JSON.stringify(Array.isArray(raw) ? raw : [])}::jsonb WHERE id = ${id}`; break;
        case 'memo':                 await sql`UPDATE recurring_invoice_schedules SET memo = ${raw as string | null}                        WHERE id = ${id}`; break;
        case 'bill_to_name':         await sql`UPDATE recurring_invoice_schedules SET bill_to_name = ${raw as string | null}                WHERE id = ${id}`; break;
        case 'bill_to_email':        await sql`UPDATE recurring_invoice_schedules SET bill_to_email = ${raw as string | null}               WHERE id = ${id}`; break;
...
export const DELETE = withAdminTracking(async function DELETE(_req: NextRequest, ctx: RouteCtx) {
  ...
  try {
    await ensureSchema();
    const sql = getSql();
    const rows = (await sql`SELECT status FROM recurring_invoice_schedules WHERE id = ${id}`) as unknown as Array<{ status: string }>;
    if (rows.length === 0) return NextResponse.json({ error: 'not found' }, { status: 404 });
    if (rows[0].status === 'active') {
      return NextResponse.json({ error: 'pause the schedule before deleting it' }, { status: 400 });
    }
