...
import { extractTrecContract, type TrecContractExtractResult } from '@/lib/server/gemini-trec-contract-extract';
...
export const POST = withAdminTracking(async function POST(request: NextRequest) {
  ...
  let result: TrecContractExtractResult;
  if (file instanceof File) {
    ...
    result = await extractTrecContract({ base64: bytes.toString('base64'), mimeType: file.type });
  } else if (typeof contractText === 'string' && contractText.trim()) {
    if (contractText.length > MAX_TEXT_CHARS) {
      return NextResponse.json({ error: 'Paste no more than 80,000 characters at a time.' }, { status: 413 });
    }
    result = await extractTrecContract({ text: contractText.trim() });
  } else {
    return NextResponse.json({ error: 'Upload a contract file or paste contract text to continue.' }, { status: 400 });
  }
  if (!result.ok) {
