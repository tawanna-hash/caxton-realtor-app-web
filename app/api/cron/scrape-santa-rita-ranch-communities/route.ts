// app/api/cron/scrape-santa-rita-ranch-communities/route.ts
//
// Vercel Cron endpoint. Santa Rita Ranch communities/neighborhoods scrape.
// One row per community (homeType='community') with structured communityData.
//
...
// Dev/preview: open.
//
// ?strip=1 — deletes ALL existing SRR community rows before upserting.

import { NextRequest, NextResponse } from 'next/server';
import { fetchSRRCommunities } from '@/lib/scrapers/santa-rita-ranch-communities';
...
const SCRAPER_SUBMITTER_NAME = 'SRR Communities Auto-Importer';
const SCRAPER_SUBMITTER_EMAIL = 'scraper-srr-communities@harmonyone.system';
...
async function runScrape(strip: boolean) {
  ...
  if (strip) {
    try {
      ...
      console.log(`[scrape-srr-communities] stripped ${stripped} existing community rows`);
    } catch (err) {
      console.error('[scrape-srr-communities] strip failed:', err instanceof Error ? err.message : String(err));
    }
  }

  const { rows, rawCount, detailFetched, detailErrors } = await fetchSRRCommunities();
  ...
  for (const row of rows) {
    try {
      ...
      console.error(`[scrape-srr-communities] upsert failed for "${row.title}" (${row.externalId}):`, msg);
    }
  }
  ...
  return {
    ...
    detailErrorDetails: detailErrors.length > 0 ? detailErrors.slice(0, 5) : undefined,
...
async function _GET(req: NextRequest) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error('[scrape-srr-communities] fatal error:', msg);
...
export const GET = withScraperRun('scrape-santa-rita-ranch-communities', _GET);
