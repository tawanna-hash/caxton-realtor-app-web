...
// SABOR (San Antonio Board of REALTORS) calendar, upserts events into
// Postgres, and prunes stale rows.
...
import { scrapeSabor } from '@/lib/sabor-scraper';
import { withScraperRun } from '@/lib/with-scraper-run';

export const runtime = 'nodejs';
// Vercel's Pro plan default is 60s; ICS fetches make this safer at 300s.
...
async function _GET(req: Request) {
  ...
  try {
    ...
    const events = await scrapeSabor(months);
    const counts = await upsertEvents(events);
    const pruned = await pruneStale('sabor', 30);

    const ms = Date.now() - startedAt;
    console.log(
      `[cron/scrape-sabor] ${ms}ms received=${events.length} ` +
    ...
    console.error('[cron/scrape-sabor] failed', err);
    return Response.json(
      {
        error: 'scrape_failed',
...
export const GET = withScraperRun('scrape-sabor', _GET);
