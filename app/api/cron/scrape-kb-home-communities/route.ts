...
async function runScrape(strip: boolean) {
  ...
  if (strip) {
    try {
      ...
      console.log(`[scrape-kb-home-communities] stripped ${stripped} existing community rows`);
    } catch (err) {
      console.error('[scrape-kb-home-communities] strip failed:', err instanceof Error ? err.message : String(err));
    }
  }
...
export const GET = withScraperRun('scrape-kb-home-communities', _GET);
