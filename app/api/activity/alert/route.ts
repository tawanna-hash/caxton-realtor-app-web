...
export const POST = withErrorHandling(async (req: Request) => {
  ...
  const metaHtml = a.metadata
    ? `<pre style="background:#f5f5f5;padding:12px;border-radius:6px;font-size:12px;overflow:auto">${escapeHtml(JSON.stringify(a.metadata, null, 2))}</pre>`
    : '';
  ...
  const html = `
    <div style="font-family:-apple-system,Helvetica,Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px">
      ...
      <table style="font-size:13px;color:#444;border-collapse:collapse;width:100%">
        ...
        ${a.url ? `<tr><td style="padding:4px 12px 4px 0;color:#777">URL</td><td style="padding:4px 0"><a href="${escapeHtml(a.url)}" style="color:#0a3d91">${escapeHtml(a.url)}</a></td></tr>` : ''}
      ...
      <p style="color:#888;font-size:12px;margin:0">
        Live dashboard: <a href="https://realtynewsnow.app/admin/activity" style="color:#0a3d91">realtynewsnow.app/admin/activity</a><br>
